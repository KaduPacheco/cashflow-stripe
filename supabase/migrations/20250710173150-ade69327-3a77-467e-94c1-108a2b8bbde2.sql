-- Auditoria completa de RLS e autorização avançada (CORRIGIDA)
-- Data: 2025-01-10
...
DROP POLICY IF EXISTS "lembretes_select_policy" ON public.lembretes;
DROP POLICY IF EXISTS "lembretes_insert_policy" ON public.lembretes;
DROP POLICY IF EXISTS "lembretes_update_policy" ON public.lembretes;
DROP POLICY IF EXISTS "lembretes_delete_policy" ON public.lembretes;
...
-- 4. Recriar políticas mais restritivas para categorias (usando userid em minúsculas)
...
-- 7. Criar tabela para logs de segurança
...
-- 8. Habilitar RLS na tabela de logs (somente admin)
