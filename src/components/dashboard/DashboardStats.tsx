...
export function DashboardStats({ receitas, despesas, saldo, transacoesCount, lembretesCount }: DashboardStatsProps) {
  return (
        ...
        <div className="space-y-6">
          ...
          <div className="border-t pt-6">
            <div className={`flex items-center justify-between p-4 rounded-xl border ${
              ...
              <span className={`text-sm font-medium ${saldo >= 0 ? 'text-blue-700 dark:text-blue-300' : 'text-red-700 dark:text-red-300'}`}>Saldo</span>
              <span className={`font-bold text-xl ${saldo >= 0 ? 'text-primary' : 'text-red-600'}`}>
          ...
          <div className="pt-4 border-t space-y-4">
            <div className="flex items-center justify-between text-sm bg-muted/30 p-3 rounded-lg">
              ...
              <span className="font-semibold text-card-foreground">{transacoesCount}</span>
            </div>
            <div className="flex items-center justify-between text-sm bg-muted/30 p-3 rounded-lg">
              <span className="text-muted-foreground">Lembretes Ativos</span>
              <span className="font-semibold text-card-foreground">{lembretesCount}</span>
            </div>
          </div>
        </div>
      </CardContent>
